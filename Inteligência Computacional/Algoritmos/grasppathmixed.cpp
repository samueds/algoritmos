...
pair<int,vector<int> > mixed_pr(pair<int,vector<int> > s, pair<int,vector<int> > slinha, vector<vector<int> >g)
{
...
int main(int argc, char** argv)
{
	...
	printf("Iteracoes %d\n", iter);
	/*for(int i = 0; i < top1.second.size(); i++)
	{
		printf("%d ", top1.second[i]);
	}
	*/
	cout << endl;
	...
	auto diff = end - start;
	printf("tw = %d\n",resp);
	fprintf(arq,"%d$%d$%f\n",n+m,resp,chrono::duration <double, milli> (diff).count());
